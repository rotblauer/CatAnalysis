...
for(i in 1:clustNums){
  print(i)
  km <- kmeans(d,centers=i,trace = TRUE,iter.max = 20) 
  tmp=data.frame(km$cluster)
  colnames(tmp)=c(cols[i])
  loc_gc=cbind(loc_gc,tmp)
  print(colnames(loc_gc))
  tmp2 = data.frame(K=c(i),WithinSS=c(mean(km$tot.withinss)))
...
ggplot(loc_gc, aes(x = lon, y = lat,colour=as.factor(clust6))) + geom_point()+ t2
ggplot(loc_gc, aes(x = lon, y = elevation,colour=as.factor(clust6))) + geom_point()+ t2
ggplot(loc_gc, aes(x = lat, y = elevation,colour=as.factor(clust6))) + geom_point()+ t2

 
# loc_gcP =loc_gc[which(loc_gc$clust6!=6),]
mfrow3d(nr = 2, nc = 1)
plot3d(loc_gc$lon, loc_gc$lat, loc_gc$elevation,col =loc_gc$clust6 )
